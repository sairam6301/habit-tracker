...
def google_callback():
    ...
    try:
        ...
        if user:
            ...
            conn.execute(
                'INSERT INTO users (username,email,password,google_id,avatar_url) VALUES (?,?,NULL,?,?)',
        ...
        payload = base64.urlsafe_b64encode(
            json.dumps({'id': user['id'], 'username': user['username'],
                        'email': user['email'], 'avatar_url': avatar_url}).encode()
        ).decode()
        return redirect(f'/dashboard?google_auth={payload}')
...
def create_habit():
    try:
        ...
        c    = conn.cursor()
        c.execute(
            'INSERT INTO habits (user_id,name,emoji,goal,color,category,difficulty) VALUES (?,?,?,?,?,?,?)',
...
def update_habit(habit_id):
    try:
        ...
        conn = get_db()
        conn.execute(
            'UPDATE habits SET name=?,emoji=?,color=?,category=?,difficulty=? WHERE id=?',
...
def toggle_completion():
    try:
        ...
        if existing:
            ...
            conn.execute(
                'INSERT INTO completions (habit_id,user_id,completed_date) VALUES (?,?,?)',
...
def export_csv():
    try:
        ...
        habits      = conn.execute('SELECT * FROM habits WHERE user_id=?', (user_id,)).fetchall()
        completions = conn.execute(
            'SELECT * FROM completions WHERE user_id=? ORDER BY completed_date', (user_id,)
        ).fetchall()
        conn.close()
        output = io.StringIO()
    ...
    <body style="font-family:'Segoe UI',sans-serif;background:#f0fdf4;margin:0;padding:20px">
      <div style="max-width:560px;margin:0 auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.1)">
        <div style="background:linear-gradient(135deg,#166534,#22c55e);padding:28px 32px">
          ...
          <p style="color:rgba(255,255,255,.8);margin:6px 0 0;font-size:14px">{datetime.now().strftime("%A, %B %d %Y")}</p>
        </div>
        <div style="padding:28px 32px">
          ...
          <div style="display:flex;gap:16px;margin:20px 0;flex-wrap:wrap">
            <div style="flex:1;background:#f0fdf4;border:1px solid #86efac;border-radius:10px;padding:16px;min-width:120px;text-align:center">
              ...
              <div style="font-size:11px;color:#6b7280;text-transform:uppercase">Completion</div>
            </div>
            <div style="flex:1;background:#f0fdf4;border:1px solid #86efac;border-radius:10px;padding:16px;min-width:120px;text-align:center">
              <div style="font-size:28px;font-weight:700;color:#166534">{habits_done}/{total_habits}</div>
              <div style="font-size:11px;color:#6b7280;text-transform:uppercase">Done Today</div>
            </div>
            <div style="flex:1;background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:16px;min-width:120px;text-align:center">
              ...
              <div style="font-size:11px;color:#6b7280;text-transform:uppercase">Best Streak</div>
            </div>
          </div>
          <table style="width:100%;border-collapse:collapse;font-size:13px">
            <thead><tr style="background:#f9fafb">
              <th style="padding:8px 12px;text-align:left;color:#6b7280">HABIT</th>
              <th style="padding:8px 12px;text-align:center;color:#6b7280">TODAY</th>
              <th style="padding:8px 12px;text-align:center;color:#6b7280">STREAK</th>
            </tr></thead>
            <tbody>{rows_html}</tbody>
          </table>
          <p style="margin:24px 0 0;font-size:14px;color:#6b7280;text-align:center">
            {"🎉 Amazing work today!" if pct==100 else "💪 Keep going — every habit counts!" if pct>=50 else "🌱 Small steps lead to big results. You've got this!"}
          </p>
        </div>
...
def send_progress_email(to_email, username, habits_done, total_habits, pct, best_streak, habit_rows):
    ...
    msg.attach(MIMEText(
        build_email_html(username, habits_done, total_habits, pct, best_streak, habit_rows),
        'html'
    ))
    try:
...
# ══════════════════════════════════════════════════════════════
#  Daily Email Job — runs every minute, sends at user's chosen time
# ══════════════════════════════════════════════════════════════
def daily_email_job():
    ...
    try:
        ...
        # Check table exists
        tbl = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='email_notifications'"
        ).fetchone()
        if not tbl:
            conn.close()
            return
        # Find users whose send_time matches current minute
        notifs = conn.execute(
            '''SELECT en.user_id, en.email, en.send_time, u.username
               FROM email_notifications en
               JOIN users u ON u.id = en.user_id
               WHERE en.enabled = 1 AND en.send_time = ?''',
            (now_time,)
        ).fetchall()
...
def set_email_notification():
    try:
        ...
        conn     = get_db()
        existing = conn.execute(
            'SELECT id FROM email_notifications WHERE user_id=?', (user_id,)
        ).fetchone()
        if existing:
            conn.execute(
                'UPDATE email_notifications SET email=?, send_time=?, enabled=? WHERE user_id=?',
                (email, time, 1 if enabled else 0, user_id)
            )
        else:
            conn.execute(
                'INSERT INTO email_notifications (user_id,email,send_time,enabled) VALUES (?,?,?,?)',
                (user_id, email, time, 1 if enabled else 0)
            )
        conn.commit()
        conn.close()
        action = 'enabled' if enabled else 'disabled'
        return jsonify({'success': True, 'message': f'Email notification {action} at {time}'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/email-notification', methods=['GET'])
def get_email_notification():
    try:
        ...
        conn = get_db()
        row  = conn.execute(
            'SELECT * FROM email_notifications WHERE user_id=?', (user_id,)
        ).fetchone()
        conn.close()
        if row:
            return jsonify({'email': row['email'], 'time': row['send_time'], 'enabled': bool(row['enabled'])})
        return jsonify({'email': '', 'time': '08:00', 'enabled': False})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def delete_account():
    try:
        ...
        if not user_id:
            return jsonify({'error': 'user_id required'}), 400
        conn      = get_db()
        habit_ids = [h['id'] for h in conn.execute(
        ...
        conn.execute('DELETE FROM email_notifications WHERE user_id=?', (user_id,))
...
if SCHEDULER_AVAILABLE:
    ...
    scheduler.add_job(daily_email_job, 'interval', minutes=1)
    scheduler.start()
    import atexit
...
if __name__ == '__main__':
    ...
    print('  ✅ Scheduler — emails sent at each user\'s chosen time.')
